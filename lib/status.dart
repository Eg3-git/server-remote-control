...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void save() async {
    for (Server s in servers) {
      await widget.storageHelper.insertServer(s);
    }
    ...
    for (Server s in servers) {
      rows.add(Row(
        children: [
          ...
          Expanded(
              flex: 2,
              child: s.isOnline ? const Text("Online") : const Text("Offline")),
          const Spacer(),
          IconButton(
            icon: const Icon(Icons.settings),
            color: Colors.white,
            onPressed: () => popup(editMode: true, serverToEdit: s),
          ),
          const Spacer()
        ],
      ));
